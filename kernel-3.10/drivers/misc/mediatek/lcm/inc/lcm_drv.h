...
#define CONFIG_DEVINFO_LCM	//add by lizhiye, for lcm info
...
typedef struct
{
	...
	void (*enable_cabc_cmdq)(void* handle,unsigned int enable);		//add by mtk
	...
	// set te
	void (*set_te)(unsigned int onoff);
	void (*set_hbm)(void *handle, unsigned int onoff);
} LCM_DRIVER;
